...
export class JiraService {
   ...
   * The comment is wrapped in ADF; the prefix `[Tool]` is intentionally NOT
   * added here, so user-edited comments stay clean.
   */
   ...
   * @param {boolean} [opts.addToolPrefix=false] - prepend "[Tool] " to the
   *   comment text. Used by the GitHub Sync flow so that
  ...
  static async createWorklog(domain, issueKey, { timeSpentSeconds, targetDate, description, addToolPrefix = false }) {
    const text = addToolPrefix ? `[Tool] ${description}` : description;

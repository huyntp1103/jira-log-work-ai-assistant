import { JiraService, extractCommentText } from './jira.js';
...
export class ReportEngine {
  ...
  async generate() {
    ...
    logIssues.forEach((issue) => {
      ...
      // Build a `Reason` from the user's worklog comments on the target date.
      // Strip the [Tool] prefix (added by GitHub Sync) since it isn't
      // meaningful for the report. Drop empties and join with "; ".
      const reason = targetLogs
        .map((l) => extractCommentText(l.comment).replace(/^\[Tool\]\s*/i, '').trim())
        .filter((t) => t.length > 0)
        .join('; ');

...
export class ReportEngine {
  ...
  async generate() {
    ...
    logIssues.forEach((issue) => {
      ...
      // Strip the [Generated by Log Work tool] prefix (added by GitHub Sync) since
      // it isn't meaningful for the report. Drop empties and join with "; ".
      const reason = targetLogs
        .map((l) => extractCommentText(l.comment).replace(/^\[Generated by Log Work tool\]\s*/i, '').trim())
        .filter((t) => t.length > 0)
        .join('; ');

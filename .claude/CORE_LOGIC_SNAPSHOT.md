...
const JiraService = {
   ...
   * Comment is prefixed with [Generated by AI] so GitHubService.isSynced can detect it.
   */
  async createWorklog(issueKey, { timeSpentSeconds, targetDate, description }) {
    return this.fetch(`/rest/api/3/issue/${issueKey}/worklog`, 'POST', {
      ...
      comment: {
        ...
        content: [{
          type: 'paragraph',
          content: [{ type: 'text', text: `[Generated by AI] ${description}` }],
        }],
      },
    });
  },
...
const GitHubService = {
  ...
  /**
   * Check whether a Jira ticket already has an [Generated by AI] worklog on targetDate for current user.

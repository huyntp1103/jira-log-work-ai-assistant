...
const CONFIG = {
  ...
  // --- Gemini ---
  GEMINI_KEY: '',         // replace with your key
...
const JiraService = {
   ...
   * Comment is prefixed with [Generated by Log Work tool] so GitHubService.isSynced can detect it.
   */
...
const GitHubService = {
  ...
  async extractTicketMap(events) {
    ...
    const resolvePrIds = async (ev) => {
      const pr = ev.payload.pull_request;
      const inline = (pr?.title || '') + ' ' + (pr?.head?.ref || '');
      const ids = extractIds(inline);
      if (ids.length > 0 || !pr?.url) return ids;
      const remote = await GitHubService.fetchPrText(pr.url, prTextCache);
    ...
    for (const ev of events) {
      if (['PullRequestReviewEvent', 'PullRequestReviewCommentEvent', 'PullRequestEvent'].includes(ev.type)) {
        const ids = await resolvePrIds(ev);
    ...
    for (const ev of events) {
      ...
      // CASE 3: PR ACTIONS
        ...
        if (['opened', 'merged', 'reopened'].includes(action)) {
          ...
          ticketIds = await resolvePrIds(ev);
        }
      }
  ...
  /**
   * Check whether a Jira ticket already has an [Generated by Log Work tool] worklog on targetDate for current user.
...
async function runGitHubSync(dateStr = null, dryRun = true) {
  ...
  const ticketMap = await GitHubService.extractTicketMap([...events].reverse());

...
1. **Jira Tasks** — Portfolio view of tracked Jira **Releases** (versions) and **Epics**.
   - User types a bare numeric id (e.g. `27643`) or a full issue key (e.g. `UP-68179`). Worker probes `/rest/api/3/version/{id}` and `/rest/api/3/issue/UP-{id}` in parallel; epic wins if both resolve.
...
### Services
- **`src/services/jira.js`** — Jira REST API v3.
  ...
  - `createWorklog` — prefixes the comment with `[Generated by Log Work tool]`.
...
- **`src/background/worker.js`** — Service worker. Message handlers:
  ...
  - `JIRA_WORKLOG_PREVIEW`, `JIRA_WORKLOG_UPDATE`

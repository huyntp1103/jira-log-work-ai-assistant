...
- **GitHub Activity Sync:** Fetch GitHub events for a date, extract Jira ticket IDs, create Jira worklogs automatically. Deduplication via `[Generated by AI]` tag check on existing worklogs.
...
- **`src/services/jira.js`** — Jira REST API v3: fetch profile, search JQL, create worklog (ADF comment format, `[Generated by AI]` prefix tag).
...
- Existing worklogs are checked for `[Generated by AI]` prefix in `comment.content[0].content[0].text`.
